...
window.onload = function () {
    ...
    <c:set var="count" value="0"/>
            dataPoints: [
            <c:forEach var="temp" items="${Temps}" varStatus="loop">
                    <c:set var="count" value="${count+1}"/>
               {x:${temp.getTime()},y:${temp.getWaterTemp()} }<c:if test="${count<Temps.size()}">,</c:if>           
            </c:forEach>
            ]
        },
        {        
            ...
            <c:forEach var="temp" items="${Temps}" varStatus="loop">
                <c:set var="count" value="${count+1}"/>
               {x:${temp.getTime()},y:${temp.getRoomTemp()} }<c:if test="${count<Temps.size()}">,</c:if>             
            </c:forEach>
            ]
        }
    ...
    <div class="container">
      <div class="row" >
        <div id="comp_box" class="col-md-12">
          ...
          <div style="width:100%"><h2 style="text-align:center;">Temperature</h2></div>
          <div class="row" style="padding: 10px;">
        <div class="col-md-2"><table><tr><td>Room Temp:</td><td>${roomTemp}°C</td></tr></table></div>
	<div class="col-md-2"><table><tr><td>Water Temp:</td><td>${waterTemp}°C</td></tr></table></div>
        <div class="col-md-2"><table><tr><td>Heater</td><td><input type="checkbox" id="toggle-button" onchange="switchRelay(1)"  <c:if test="${Pins[0] == 'HIGH'}">checked</c:if>></td></tr></table></div>
	<div class="col-md-2"><table><tr><td>Fans</td><td><input type="checkbox" id="toggle-button2" onchange="switchRelay(2)"  <c:if test="${Pins[1] == 'HIGH'}">checked</c:if>></td></tr></table></div>
        <div class="col-md-2"><table><tr><td>Pref. Temp.</td><td><select class="form-control">
                          <option>12</option>
                          <option>13</option>
                          <option>14</option>
                          <option>15</option>
        ...
        <div class="col-md-2" ><table><tr><td><button type="submit" class="btn btn-default">Apply</button></td></tr></table>
        </div></div>
          <div class="row">
              <c:if test="${not empty Temps}">
              <div id="chartContainer" style="height: 300px; width: 100%;"></div>
              </c:if>
              <c:if test="${empty Temps}">
              THERE ARE NOT TEMPS!!!
              </c:if>
          </div>
          <div class="row">
            <table class="table table-striped">
                ...
                <tr>
                    <td>${temp.getDate()}</td>
                    <td>${temp.getRoomTemp()}°C</td>
                    <td>${temp.getWaterTemp()}°C</td>
                </tr>
                </c:forEach>
